...
1. `/research:rerun business-model disqualifier-scan ORCL` — Item 2 (>50% promoter/insider pledge check) currently states "no pledge disclosure of any kind found... effectively 0%"; the proxy supplies an actual, computable pledge figure. Verdict likely stays cleared (well under the 50% threshold), but the evidence basis is factually wrong today. Cascades to: business-model, earnings, balance-sheet-survival, management-governance, valuation, catalyst.
...
This is the fourth intake scan run today against the same finished run and decision fingerprint (`2026-08-14_intake_plan.json`, `_v2`, `_v3` precede this file). No pool document has changed since `_v3` (scanned 2026-08-14T06:25:49.756Z) — the same two documents are found again with identical SHA-256 hashes, and this scan (2026-08-14T06:31:01.433Z) independently re-verified the pledging disclosure, the current run's contradicted findings, and the incentives-and-compensation partial-data cap directly against the source documents before reproducing the same scoped plan.
