# Model Portfolio — 2026-07-18

**Scope:** all (standing, corrected decision ledger, 5 decisions)
...
**Why zero gross:** none of the 5 standing ledger records reached the **Selected** basket (Strong Buy / Buy / Starter Position Only) required for long eligibility under Step 2 of this process. Four are **Watchlist** (AMZN, BG, EMAAR, TMCV) and one is **Rejected/Avoid** (HCG). CLAUDE.md §18 bars forcing a Buy, and the sizing math (fractional-Kelly on edge × downside, scaled by conviction) self-zeroes any name with a negative expected return or no evidence-backed edge — that is the case for all five here. This is a valid, decision-useful output, not a gap in the process.

**Concentration / correlation:** not applicable with zero positions. Flagged for future reference: AMZN and EMAAR both carry a governance/owner-alignment cap; BG and TMCV both carry a Filter 4 large-deal / serial-acquirer cap (Viterra integration for BG, Iveco financing for TMCV). If either pair later clears its cap and both enter the book, they should be treated as correlated on that shared risk vector, not as two independent bets.
