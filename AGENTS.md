...
- **Code — everything else.** Engine source (`ui/server/src/**`, `ui/web/**`), the prompt-program (`.claude/agents/**`, `.claude/commands/**`, `frameworks/**`), the doctrine files (`AGENTS.md` and its twin), build, CI, and scripts (`scripts/**`, `.github/**`), and root `*.md`. **Code MUST go through branch → pull request → all required CI → multi-view adversarial review. It may NOT be pushed directly to `main` by any contributor, human or AI.** The agent, command, and framework `.md` files ARE the program — a bad edit changes research behaviour with no compiler to catch it — so they are gated exactly like source code. Once the ready state is proven, only the owner's separately configured scheduled merge routine may merge it through the same protected path; interactive coding agents never merge it.
...
**Merge authority is narrow and preconfigured.** The engine's PR agent owns preparation of a code PR end to end: it may update its branch, resolve conflicts, run tests and multi-view adversarial review, triage every finding, and push corrections to that PR. Its normal terminal state is an open, green, reviewed PR. Interactive Claude, Codex, human-assistant, and review sessions have **no standing authority** to merge or take a production action. The owner's separately configured scheduled merge routine is the sole standing merge actor. It may run once or twice per day and may merge only when the branch is current, all five required PR jobs are green, every automated review has completed, every material finding is fixed or explicitly resolved, and the protected GitHub path accepts the merge without bypass. That protected merge is the release decision: the watcher then deploys automatically only after independently proving the exact `main` push and all five required jobs green. Manual/bootstrap deployment, restart, configuration, canary, and run mutation remain separate exact authorizations. An implementation request or words such as "continue", "go ahead", "fix it", "done?", and "all done?" never let an interactive agent impersonate the scheduled routine.

**Environment boundary.** All implementation and verification stays in the feature worktree and local or staging environments. Production may be inspected read-only to diagnose a reported problem. An interactive agent must not merge to the production branch, manually deploy, restart services, change production flags or configuration, or launch, retry, resume, cancel, or otherwise mutate a production run without exact current authority for that production action. After the scheduled routine merges a ready PR, the exact-CI watcher—not the coding agent or merge routine—owns deployment, run draining, health verification, rollback, and audit. Ongoing work belongs to its operator and must not be interrupted by code delivery. If a reviewed PR is ready, report `PR ready for the scheduled merge routine; not merged or manually deployed` and stop.
...
2. A GitHub branch ruleset on `main` requires a pull request and the same five CI jobs the deploy watcher requires. Neither humans nor coding agents are bypass actors. The engine GitHub App is the sole bypass identity and `commit-run.sh` constrains it to audited data paths; that capability is not merge authority for code. The scheduled merge routine uses the protected PR path, never the bypass. The exact GitHub settings and contributor workflow live in `CONTRIBUTING.md`.
...
An interactive code task is "done" only at an open, green, reviewed PR tested on local or staging; it must
not proceed to merge or any manual production action. The owner's scheduled merge routine may merge that ready
PR only through the protected path. That merge delegates only its exact automatic release: the watcher must re-run all five jobs on the resulting `main`
