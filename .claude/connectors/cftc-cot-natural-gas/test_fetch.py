...
spec = importlib.util.spec_from_file_location("cftc_gas_fetch", os.path.join(HERE, "fetch.py"))
...
def row(index: int) -> dict:
    ...
    return {
        ...
        "m_money_positions_long_all": str(300000 + index), "m_money_positions_short_all": "200000",
        "prod_merc_positions_long": "250000", "prod_merc_positions_short": str(550000 + index),
    }
...
assert payload["observations"][-1]["managed_money_net"] == 100159
assert payload["observations"][-1]["producer_net"] == -300159
for bad in (
    rows[:149],
    [*rows[:159], {**rows[-1], "contract_market_name": "NATURAL GAS ICE HENRY HUB"}],
...
print("PASS: cftc-cot-natural-gas")
