...
spec = importlib.util.spec_from_file_location("cftc_crude_fetch", os.path.join(HERE, "fetch.py"))
...
def row(index: int) -> dict:
    ...
    return {
        ...
        "report_date_as_yyyy_mm_dd": day + "T00:00:00.000", "open_interest_all": "1800000",
        "m_money_positions_long_all": str(250000 + index), "m_money_positions_short_all": "100000",
        "prod_merc_positions_long": "500000", "prod_merc_positions_short": str(750000 + index),
    }
...
assert payload["observations"][-1]["managed_money_net"] == 150159
assert payload["observations"][-1]["producer_net"] == -250159
for bad in (
    rows[:149],
    [*rows[:159], {**rows[-1], "contract_market_name": "BRENT CRUDE OIL LAST DAY"}],
