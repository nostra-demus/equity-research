...
- **STRIP the verdict — never ingest or cite it (§24).** Drop and never carry forward the analyst's Rating (BUY/SELL/HOLD), Target Price, Upside/Downside, and their beat/miss framing ("vs *our* estimate of X"). The engine forms its own verdict; a sell-side recommendation is banned marketer input (§21/§24), not evidence. (Extraction stays verbatim per §27 — the note is transcribed whole; the STRIP happens here, at the reading layer, not in the extractor.)
  - *Worked example — FabResearch "UAE Equity Research — Emaar Properties 4Q25":* STRIP `Current Price AED 16.30 / Target Price AED 19.25 / Upside +18% / Rating BUY` and "net profit … *higher than our estimate of AED 4,385 Mn*". KEEP the **"Earnings Call Summary"** bullets *from that same 4Q25 note* — property sales AED 80 Bn (+15.6% YoY), backlog AED 155 Bn (+39%), 50,800 units under construction — as *management commentary via the call*, each cross-checked against the results. (Do NOT pull a detail from a different quarter's note into this one — that would be the §15 period-mix the whole rule guards against.)
...
| No VERBATIM transcript, but a sell-side / analyst earnings note IS present | 02, 03, 04, 06 | Use the note as a VERDICT-STRIPPED transcript proxy for commentary (Transcript Sourcing & Fallback rule): drivers/guidance colour from its call-summary, numbers from the primary doc, tone/candor NOT assessable |
