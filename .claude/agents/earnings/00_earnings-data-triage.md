...
3. Classify each file by earnings-relevance: annual filing, quarterly filing, transcript, investor deck, consensus/estimate export, cash flow data, guidance data, data export, user note, other. **For a transcript, record its SUBTYPE and source tier** (per `MODULE_RULES.md` → Transcript Sourcing & Fallback), because they are trusted differently downstream:
   ...
   - **company earnings press release / results intimation** — a primary disclosure; the numbers/official-guidance anchor, thin on driver colour.
...
| No VERBATIM transcript, sell-side proxy present | | 02, 03, 04, 06 | proxy = verdict-stripped commentary; clarity ≤65; tone/candor not assessable |
