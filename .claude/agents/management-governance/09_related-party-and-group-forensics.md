...
17. **Reconcile 07's full network against the RPT note and the subsidiary/associate list (disclosure integrity).** Every discovered entity that transacts with the listco must appear in the RPT disclosures — including entities 07 reached by brand lineage, former names, or the founder loop, not only those reached by directorship. A transacting-but-undisclosed entity propagates RF-PPL-005 and is often more decision-relevant than the transaction itself — the non-disclosure outweighs the item.
