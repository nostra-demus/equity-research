...
7. **Round-tripping cross-year name match (A5-04, hard step).** Build ONE list of every named related counterparty across ALL years read. Match customer names against vendor names — across years, not just within one year. The same name appearing as BOTH customer and vendor is the DHFL/Gensol circle: money that leaves as a purchase and returns as a sale. Any match is Red pending an operating explanation that actually holds (a genuine two-way trade exists — a supplier that also buys scrap — but it must be explained and priced, not assumed).
