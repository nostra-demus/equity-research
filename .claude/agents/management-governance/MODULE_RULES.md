...
- **Exact-resume boundary overrides cross-run refresh; frozen continuation does too.** When `NOSTRA_CONTINUATION_RUN_ROOT` is set, the cockpit has frozen one exact run root. Agents `07`, `12`, and `99` MUST NOT Glob, search, or read a prior-dated management-governance folder, and every specialist that self-resolves balance-sheet-survival (`10`, `11`, or `12`) may use only the copy inside that exact continuation root. When `NOSTRA_EXACT_MODULE_RESUME=1`, the boundary is stricter: an optional current-root module must also be named in `NOSTRA_EXACT_MODULE_INPUTS`. If the permitted current-root input is absent, proceed from the data pool and state that the cross-module input was unavailable. These safety boundaries override the ordinary delta-refresh rule above.
