...
- **Breadth budget (the hop cap bounds DEPTH, not WORK).** A controller with dozens of current and historical directorships produces an arbitrarily wide hop-1 set, and branch-local termination no longer contains it. So bound the width too, deterministically: **at most 25 hop-1 entities and 15 hop-2 entities enter a full Phase-2 sweep**, selected in this fixed priority order — (1) lineage / predecessor entities, (2) entities that transact with the listco or appear in the RPT note, (3) entities controlled by a Tier-A person (controller, CEO, CFO, chair, CS), (4) entities carrying any adverse hit from their scoped pass, (5) entities controlled by Tier-B people, (6) everything else, by recency of the linkage. Ties break by the subject's own registry status (struck-off / under insolvency first). Everything above the limit is **not dropped** — it goes on the Scope-Boundary Register by name with reason `breadth budget`, and the coverage cap applies. State the limit used and the count that overflowed; a run that silently sampled its own network is worse than one that admits where it stopped.
...
| RF-NET-003 | Live hop-1 cross-link from a register person or the listco to an entity carrying EITHER (a) a **Disqualifying-equivalent** fact — proven fraud, active debarment, sanctions match, or fugitive/absconder status — OR (b) a **Material-equivalent** fact — admitted CIRP or liquidation, live enforcement, or a credible fraud allegation (A17-07). The two fact sets are exactly those in the Transitive-exposure grading rule and the two banded Score Cap rows, so the flag, the grade and the cap always cover the same facts. Record which band fired, with the exposure basis named — never as a personal finding (§3) |
...
| **Live hop-1 cross-link to an entity carrying a Material-equivalent fact** — admitted CIRP or liquidation, live enforcement, credible fraud allegation (RF-NET-003) | People & network integrity max 50; Governance risk floor 55; conviction cap — no rating above "Watchlist". *The two RF-NET-003 rows mirror the two bands of the Transitive-exposure grading rule, so the flag and the cap can never disagree about severity* |
