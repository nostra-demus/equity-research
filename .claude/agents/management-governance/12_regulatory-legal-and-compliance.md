...
1A. **Sweep every name the company has ever traded under, not just its current one (Hard Rule).** Build the name list from `07`'s outputs, **restricted to the TARGET COMPANY's own lineage** — Section 0A (the listco's former names and its predecessor entity), Section 3B (Lineage & Phoenix Read), and, in `entity_network.json`, the listco's own `former_names` plus any subject whose `discovery_method` is `brand lineage (self-disclosed)` or `name-change trail`.

**Do NOT sweep the former names of unrelated network subjects into this register.** `entity_network.json` also carries past-directorship companies, founders' other ventures, counterparties and address-cluster leads; those entities have their own former names, and running the company sweep against them would pull a stranger's enforcement and litigation into THIS company's legal register — a false attribution at the entity level, the twin of the namesake error at the person level. Record the source relationship against every name you do sweep (`former name of the listco` / `predecessor of the listco`), so the register shows why each name belongs to this company. If `07` did not run, fall back to the registry's own previous-names field and say that the lineage-derived names could not be included. **Reuse `07`'s Sweep Log rows for anything it already queried on those entities rather than re-running them** — cite its rows, and add only the legal/enforcement queries it did not run. Run the **full company sweep separately against each former name and each predecessor entity**, with its own Sweep Log rows. Enforcement, litigation, defaults and exchange actions are indexed under the name that was current when they happened — a rename hides all of it from a present-name search, and reporting "no records" after searching only the current name is a bad-extraction error (§20), not a clean result. If the company has never been renamed and has no predecessor, log that as a checked fact with the source, not as a silent absence.
