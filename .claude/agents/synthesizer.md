...
4C. **Calibration feedback check (per `frameworks/DECISION_LEDGER.md` §18 — Phase 6).** Take the latest calibration summary read in INPUTS item 7. If none exists: `calibration_feedback.status = "not_available"`, no adjustment — state this plainly. If one exists but its `"verdict"` starts with `"Pre-data"`: `status = "pre_data"`, no adjustment. Otherwise run BOTH of the following checks (neither is optional — a module can be clean while the forecast types this thesis leans on are not, or vice versa):
  ...
  - If **any** module OR **any** forecast type is flagged, apply a single fixed **8-point confidence haircut** — one bounded constant shared by both triggers (never additive: a module flag plus a forecast-type flag still costs 8 points, not 16), set `status = "applied"`, list every flagged module in `modules_flagged` and every flagged forecast type in `flagged_forecast_types`, each with its Brier/hit-rate numbers. If both checks ran against real signal and neither flagged anything, set `status = "checked_no_action"` — record that the check ran even though it found nothing, so a clean check is distinguishable from a silently skipped one. Never let this step *raise* confidence — a module's or forecast type's good track record on other theses is not evidence for this one. Carry the full `calibration_feedback` object (including `flagged_forecast_types`) into `decision_record.json` (schema in §18) and the haircut (if any) into the Confidence Scoring Rules below.
...
- `calibration_feedback` is a JSON **object** with exactly the six keys shown in the schema above (`source_summary`, `status`, `haircut_points`, `modules_flagged`, `flagged_forecast_types`, `rationale`); `status` must be one of the four literal strings — never a paraphrase. `status=="applied"` requires `modules_flagged` and/or `flagged_forecast_types` to carry at least one entry between them.
