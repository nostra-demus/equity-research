...
9. **Prior decision memory for this same ticker** — find the newest earlier `analyses/{TICKER}_*/decision_record.json`, then the latest append-only `reviews/*_decision_review*.json` inside that run. Read only the frozen original call and the review's `decision_quality`, `error_taxonomy`, `lessons`, `learning`, `action_now`, `confidence_update`, and `next_check` fields. This is PROCESS MEMORY, not a source for current company facts: it may tell you which assumption this run must test, but every factual answer still needs evidence in the current run. Never copy a post-outcome fact into the original decision-date evidence set, and never rewrite what the earlier rating was. If no prior review exists, state that the same-company memory check has no data yet.
