...
5. Solve for the break points — the EBITDA decline `h` (a fraction, so stressed EBITDA = EBITDA·(1−h)) at which the structure first breaks. **Show the executed solve** (F09). The covenant break-point is **direction-aware** — the same MIN/MAX split as the covenant headroom in `04` (hold net debt and interest constant unless a stress also moves them):
