...
3. **Net debt** = gross debt − cash & equivalents — the **strict** basis (CLAUDE.md §15). Where liquid short-term investments are also netted in, that is the **broad** basis: label it, and show the strict figure beside it — never present a broad figure as bare "net debt" / "net cash". State the definition used; flag restricted or trapped cash. `01` designates ONE basis as the module's canonical net-debt figure (strict by default; broad only with a stated reason); every downstream agent in this module uses that designated figure, labelled — switching basis mid-module requires an explicit one-line reason.
4. **Leverage:** show BOTH `gross debt / EBITDA` and `net debt / EBITDA`. Label the EBITDA basis. If leverage is quoted on adjusted EBITDA, also give the GAAP-EBITDA-based ratio for the same period. **Label the cycle position of the EBITDA, too.** Where `business-model/10_external-dependency` or `07_business-quality` flags the company as cyclical, show leverage on BOTH the latest-year EBITDA AND a normalised / mid-cycle EBITDA (a multi-year average of the company's own EBITDA, or the cycle-midpoint from `earnings/01_historical-financials`; for a young entity with under one full standalone cycle — a recent demerger/IPO — use the predecessor entity / segment / industry mid-cycle EBITDA and name it), each labelled "peak / latest" vs "mid-cycle / normalised." A single-year leverage figure for a cyclical, headlined without its normalised counterpart, understates fragility — peak-EBITDA leverage is a floor, not the central estimate.
...
8. **Liquidity runway (in months)** = liquidity ÷ **monthly** net cash burn. Compute the burn on ONE of two clean bases — never mix them, because FCF (= CFO − total capex, §15) already nets operating inflows, **cash interest paid** (inside CFO), and **all capex**:
   - **Net-of-FCF basis (use when FCF is meaningful/positive):** monthly net burn = [ (12-month debt maturities + committed dividends/buybacks) − FCF over those 12 months ] ÷ 12. Only include obligations NOT already inside FCF (financing outflows + discretionary returns). **Do NOT also add cash interest or maintenance capex here — FCF already carries both; adding them double-counts, overstates burn, and understates runway.**
   - **Gross-obligations basis (use when FCF is negative or unreliable — most conservative):** monthly burn = [ 12-month maturities + cash interest + maintenance capex + committed dividends ] ÷ 12, with **no FCF subtraction** (you are deliberately ignoring operating inflows). Here the full obligations bucket is correct precisely because FCF is not netted against it.

   **Units matter:** dividing liquidity by the *annual* burn (the bracketed 12-month figure) gives a coverage **multiple** (e.g. 2.0× = two years covered), NOT months — to express as months, ×12 (2.0× → 24 months). Never label the bare multiple "months" (it understates runway ~12×). If FCF covers all foreseeable obligations, state a surplus / no finite runway, not a small number.
