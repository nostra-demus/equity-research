...
4. For each balance-moving origin, bridge gross production to **pre-policy exportable availability**:
   gross production + releases/recycling/imports available to the origin − domestic consumption/industrial
   use − minimum operating/strategic stock build = pre-policy exportable supply. Show every row in the
   commodity's own physical unit and name the residual. Do not subtract sanctions, export controls or
   rerouting here; `commodity-supply-security` owns those adjustments.
...
| Origin | Gross production | + releases/recycling/import availability | − domestic absorption | − stock build | Pre-policy exportable supply | Residual/gap | Source/period |
|---|---:|---:|---:|---:|---:|---|---|
