...
4. **Calibration feedback check (the commodity-scoped twin of `frameworks/DECISION_LEDGER.md` §18 — Phase 6).** Take the latest calibration summary read in RUNTIME INPUTS. This step now carries TWO independent triggers — the original hit-rate trigger, plus the error-taxonomy trigger (mirroring `frameworks/DECISION_LEDGER.md` §18 step 6, the research-swarm extension landed 2026-07-29 that this commodity twin predates and did not originally carry):
   ...
   - **Resolve `status`.** If no calibration summary exists at all: `status = "not_available"`, no adjustment, `error_defense_evidence = {}` (nothing to check yet). Otherwise a summary exists (Pre-data or real): if the commodity is flagged (hit-rate trigger) OR any category is flagged (error-taxonomy trigger), apply a single fixed **8-point confidence haircut** (never additive, never below 0, never stacked even when both triggers fire) to the `confidence` you would otherwise have set, and set `status = "applied"`. If neither trigger fires, `status = "checked_no_action"` — this is also how a Pre-data-but-error-taxonomy-active summary resolves cleanly: the hit-rate trigger is inapplicable below its own floor, but the error-taxonomy check ran and found nothing to flag, so status is `"checked_no_action"`, never `"pre_data"`. `status = "pre_data"` now applies only when the as-of summary's verdict starts with `"Pre-data"` AND no error-taxonomy category is currently leading (count ≥ 2) — the one case where genuinely nothing yet has a usable signal. Never let this step *raise* confidence — a clean track record (or a clean error-taxonomy tally) is not evidence for THIS thesis (§12 `CLAUDE.md`: high scores require specific, cited evidence for this call). Carry the full `calibration_feedback` object, including `leading_error_categories_flagged` and `error_defense_evidence`, into `decision_record.json` (shape below).
