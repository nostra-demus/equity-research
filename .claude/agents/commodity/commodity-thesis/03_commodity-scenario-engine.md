...
4. Pass the span audit from `03_commodity-volatility-distribution.md`: bear reaches the empirical lower
   bound; bull reaches the upper bound; the killer-risk outcome covers the relevant tail/event bound.
   If an anchor prevents this, explain the structural reason with evidence. Otherwise mark the set `FAIL`.
...
- Bear downside = __%; base/bear risk/reward = __.
