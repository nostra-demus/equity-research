...
For softs, macro owns only the broad USD, producer/importer FX, global-activity context where the profile
declares it, and non-policy freight/input-cost context. For sugar specifically, do not emit or score crude,
crude/sugar, ethanol parity or realised mill allocation: `commodity-cross-asset-regime` owns crude/sugar as
one confirmation cluster, `commodity-demand-inventory` owns realised cane allocation and use,
`commodity-cost-curve` owns the ethanol-parity range, and `commodity-supply-security` owns dated export and
biofuel policy. A weak BRL may remain a macro producer-FX driver, but it cannot carry any of those physical
votes.
