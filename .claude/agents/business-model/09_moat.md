...
- **Source the cost of capital in this priority, and never invent it:** (1) a company-disclosed figure only when it matches the **same entity / group scope, cash-flow geography and currency, tax basis, and method** as the return being tested; (2) otherwise a clearly-labelled group estimate — CAPM (risk-free rate + beta × equity-risk-premium), inputs shown, marked *"Inference, not from filings"* per `CLAUDE.md` §3; (3) if neither is possible, write *"cost of capital not determinable from available data"* and mark the economic test **Not assessable**. A named CGU / subsidiary impairment rate, lease rate, or pension rate is not the listed group's cost of equity or WACC and may appear only as a labelled comparator. Fabricating or scope-promoting a number to force the test is a hard error.
...
- **Match periods as well as scope.** For a financial, show the through-cycle group ROE and the latest / forward group ROE beside the same-period group cost-of-equity read. A loss-inclusive history may govern durability, but it may not be presented as the current economic spread without also printing the current/forward spread and explaining the difference.
...
- [ ] The rate and return match on entity/group scope, geography/currency, tax basis, method, and period; any CGU / subsidiary / obligation rate is comparator-only, and a financial shows both through-cycle and latest/forward group ROE spreads.
