...
1. **The subject pool's external area — `data/{SUBJECT}/external/<provider>/`.** The only path this module reads. Its manifest row carries `external: true` and (when a sidecar exists) `provenance`. See `frameworks/EXTERNAL_DATA.md` for the `peer_transcript` source-type mapping (tier 6, about the named peer).
...
2. **A peer's OWN pool — `data/<PEER>/` — is NOT in this run's audit corpus.** `extract_pool.py` (MODULE_PIPELINE Step 1.5) and `verify-evidence` run ONLY against `data/{SUBJECT}/`, so a peer call that lives only in a sibling `data/<PEER>/` pool never enters this run's `_pool_extracts` manifest or the verification corpus — its quotes and numbers would be reported as unsupported and leave the thesis provisional. A sibling-pool call is therefore a POINTER for locating the file, not a citable source in this run. To USE it, copy or route it into `data/{SUBJECT}/external/<provider>/`, where it is extracted and auditable. A claim taken from a sibling pool that has NOT been copied into the subject's `external/` area is unverifiable: flag it as such, do not let it lift the read-through weight, and prefer routing the call into `external/` over citing it in place.

**Auditable-corpus rule (hard).** Every peer quote or number a specialist cites MUST trace to a document under `data/{SUBJECT}/` (top level or `external/**`) so it sits in the run's `_pool_extracts` manifest and the `verify-evidence` corpus. A peer call reachable only from outside `data/{SUBJECT}/` is not evidence for this run until it is routed into the subject's `external/` area.
