...
5. **Complete and LOCK `{RUN_ROOT}/thesis_record.json`:**
   - Append `M0_6_1` … `M0_6_6` blocks, transcribing the specialists' fields faithfully (consensus block incl. analyst_rating_distribution + missing_reasons — estimate dispersion is NOT in the consensus block, it lives in M0_6_2; the five market blocks, incl. estimate dispersion; variant block; mispricing block; trigger block; your scoring block with the printed formula).
