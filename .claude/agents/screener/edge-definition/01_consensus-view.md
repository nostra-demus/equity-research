...
description: M0.6.1 — writes the consensus view sympathetically (numeric anchor + 2-4 key assumptions), documents any public contrary view, rates consensus unanimity, and pulls the analyst rating distribution (buy/hold/sell) that evidences it. Estimate dispersion is owned by M0.6.2 (market-implied), not duplicated here.
...
- pull estimate dispersion — that is M0.6.2's job (Block 1). You own the *belief* (what the market thinks and why, including the analyst recommendation split); M0.6.2 owns the *priced* view (dispersion, revisions, implied scenario, options, positioning). Do not re-pull dispersion here (it produced a contradictory second copy from a different source).
...
6. Pull `analyst_rating_distribution` (buy / hold / sell counts, total, buy %, source, as_of) — the recommendation split directly evidences the unanimity rating. Every unfindable field gets a `missing_reason` naming what was searched. (Do NOT pull estimate dispersion — M0.6.2 owns it.)
...
- **ratings_interpretation:** (what the recommendation split implies for the unanimity rating — estimate dispersion is NOT here; M0.6.2 owns it)
