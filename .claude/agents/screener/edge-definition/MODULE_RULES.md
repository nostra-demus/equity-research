...
- **M0.6.1 — consensus, written sympathetically.** The dominant market view in the voice of a consensus holder — not a strawman (`strawman_risk_check` locked true). At least one `numeric_anchor`. 2–4 `key_consensus_assumptions` — these are what M0.6.3 must specifically attack. The contrary view: if one exists publicly, who holds it and their number; if none found, say so and when it was checked. Unanimity rating (high / moderate / low) with one-sentence evidence, backed by the analyst buy/hold/sell recommendation split (`analyst_rating_distribution`) where findable (else missing_reason). (Estimate dispersion is NOT pulled here — it lives in M0.6.2, Block 1; duplicating dispersion produced a contradictory second copy from a different source.)
...
- **M0.6.4 — mispricing reason.** Primary category from exactly: structural / mandate_constraint / complexity / timing / behavioral. THREE separately-labelled, independently verifiable facts — each NEW (evidence for the persistence mechanism of the chosen category, e.g. the index/flow/forced-seller data, the specific mandate or ratings floor, the model-update lag), NOT a number already on the record in M0.6.1/6.2/6.3 (a price target or dispersion figure is the thing being explained). Assertions are not facts. Secondary categories each carry their own rationale.
