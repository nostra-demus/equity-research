...
8. **Falsification / kill-switch attack (`M0_5`) — the single most important check.** Take `monitorable_threshold_rate` + `_unit` + `_date`, and the `M0_4` horizon/expiry. Ask: given a plausible bear-case path (not the worst conceivable case — a REALISTIC one), would the threshold actually be crossed and observed before the horizon closes? Two failure modes to hunt for specifically:
   - **Fireproof by magnitude** — the threshold is set so far from any plausible near-term move that it could never fire in time (e.g. a decline threshold larger than the metric's own historical volatility over the horizon window).
   ...
   Set `is_fireproof: true` if either applies, with the specific numbers that show it.
