...
2b. Read `03_news-impact-sizing.md` if present (it sizes the move the originating news justifies on fundamentals — `frameworks/screener/NEWS_IMPACT.md`). For each candidate it sized, carry its read into the candidate's `news_impact` block (below) and onto the deck card. It is a **fundamentals floor, not a verdict** — never let it change the ranking, the routing, or the exposure score. If the agent did not run (e.g. a diffuse signal with no issuer to size), omit the block; do not invent one.
3. **Write `{RUN_ROOT}/candidates.json`** per `frameworks/screener/candidates.schema.json`: thesis_id, signal_id, generated_at, routing (restated from the locked record), and the ranked `candidates` array (candidate_id, ticker, company_name, exchange, side, beneficiary_ref, exposure_score, exposure_rationale, liquidity_note, prior_coverage {has_run, latest_run_root, latest_decision, data_pool_present}, pair_with, caveats, optional `news_impact` {applicable, mode, missing_reason, recurrence, event_quantification, delta_value, implied_move_pct, observed_move_pct, gap_read, caveats, sources} folded verbatim from `03_news-impact-sizing.md`, sources). Validate it parses.
...
- News-impact: {implied {±X%} vs observed {±Y%} → {priced | underpriced — flag for research | re-rate to judge}; or omit if not sized} (a fundamentals floor, not a verdict)
...
- [ ] Any `news_impact` block is folded verbatim from `03_news-impact-sizing.md` — it never changed the ranking, routing, or exposure score.
