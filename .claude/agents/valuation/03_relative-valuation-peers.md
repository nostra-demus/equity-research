...
5. Compute the company's premium/discount to the peer median on each multiple: `premium/discount = (company multiple − peer median) / peer median` — positive = premium, negative = discount. Divide by the **peer median** (the reference), never by the company's own multiple.
