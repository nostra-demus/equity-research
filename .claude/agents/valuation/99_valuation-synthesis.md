...
- First model, `ev_bridge`: `{model: "ev_bridge", ev, net_debt, minority, other, shares, stated_drivers, source}` — `(ev − net_debt − minority + other) / shares` MUST reproduce the scenario's `level` within the guard tolerance (REPRODUCE-or-omit, exactly like the v1.1 method internals). The bridge terms are THIS scenario's own (a trough bridge may differ from the top-level `net_debt` basis — follow the orb and cite it in `source`).
