...
**Escalation (state which branch you took, in one line).** If the model WACC sits more than ~3pp BELOW a **scope-matched group rate**, or below ~two-thirds of the market-implied rate, the model rate is **presumed wrong**. Take one of: (a) rebuild it with the floors above; (b) re-run the DCF at that scope-matched group rate and publish THAT as the base intrinsic value, with the CAPM build shown beside it; or (c) mark this DCF a labelled cross-check, tell `07` to hold it at the cross-check weight cap, and say why in one sentence. A mismatched CGU / subsidiary / obligation rate cannot trigger branch (b). Publishing the low-rate value as the headline with the divergence noted only as a caveat is NOT one of the options. **When this trigger fires, add the standalone tag line immediately after (no other text before the tag on that line): `RF-VAL-003: model WACC presumed wrong — escalation branch (a|b|c) — {one-line reason}`** (write the actual letter taken, e.g. `escalation branch (b)`) — this is how `scripts/rating_caps.py` check BD (`eval_bd_cost_of_capital_reality_test`) verifies the escalation was actually followed; prose alone is not machine-checkable, exactly like the sibling §16 Sector Cycle Reality Test's `RF-VAL-001`/`RF-VAL-002` tags that check BB reads.
...
- [ ] **§3A Cost-of-Capital Reality Test table is filled.** Every disclosed rate records valuation object, geography/currency, tax basis, and method; only a five-way scope match may anchor the group valuation. A mismatched CGU / subsidiary / obligation rate is comparator-only. Where the model WACC is >~3pp below a scope-matched group rate, or below ~two-thirds of the market-implied rate, the escalation branch taken is named in one line **and carries its standalone `RF-VAL-003` tag line naming that branch — prose alone does not let `rating_caps.py` check BD verify the escalation was followed.**
