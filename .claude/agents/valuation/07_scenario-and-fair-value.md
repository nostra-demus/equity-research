...
When it fires, compute an **explicit structural-reset fair value** and show the bridge — never state the floor as a bare range. Build it from a **reset (impaired) earnings base** = a structurally-lower normalized margin × revenue (name the impaired margin and the reason — share loss to a new equilibrium, demand runoff, terminal-value fade; use `04`'s declining-perpetuity terminal where it fired) × an **impaired (de-rated) terminal multiple** the disrupted business would actually warrant, bridged to per-share with `01`'s net cash and share count in an executed Bash/Python snippet (command + result shown). The published structural-reset per-share MUST reconcile to those inputs — `impaired earnings × impaired multiple ± net cash ÷ shares`. A number that does not tie to its own stated multiple and earnings base is a defect, not a floor.

**Which case it becomes (graduated, not blunt).** If the moat verdict is confirmed **No moat proven** or **eroding**, the structural-reset IS the headline Bear — the permanent-impairment path is the more likely down-leg. If the trigger fired ONLY via the disruption-flag (`07_business-quality` ≤ ~40) while the moat is still **intact** (e.g. *Narrow, stable*), keep the headline Bear at the cyclical through-cycle trough for the stated horizon and carry the explicit structural-reset to §24 / Kill Criteria as the labelled **avoid-ruin floor** — it is the multi-year permanent-impairment scenario, not the 12-month bear. Either way it is computed, traceable, and reaches §24; only its billing (headline Bear vs avoid-ruin floor) turns on whether the moat has actually broken. Label which the Bear reflects and why. (Still no probabilities — that is the master synthesizer's job.)
...
- [ ] If the structural-reset / permanent-impairment trigger fired, the structural-reset per-share was produced by an executed snippet and reconciles to its stated impaired-earnings × impaired-multiple ± net-cash inputs — billed as the headline Bear only on a confirmed No-moat / eroding verdict, otherwise carried to §24 as the labelled avoid-ruin floor.
- [ ] No banned phrases.
