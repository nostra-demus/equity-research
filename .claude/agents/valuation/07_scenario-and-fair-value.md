...
**Structural / permanent down-leg (avoid-ruin — distinct from the cyclical trough).** A through-cycle trough is *recoverable by construction*; it cannot represent equity permanently impaired while the company stays solvent (a broken moat, structural demand collapse, terminal-value evaporation — the Kodak/Nokia archetype). When `business-model/09_moat.md` returns the verdict **No moat proven** OR an **eroding** moat trajectory, OR `business-model/07_business-quality.md` scores its **rate-of-change / disruption** row **≤ ~40** (high disruption — its §24 Filter-5 threshold), the Bear level must be the WORSE of (a) the through-cycle trough above and (b) a **structural-reset** level — normalized earnings and the terminal multiple reset to a new, lower, non-recovering base (use `04`'s declining-perpetuity terminal where it fired). Label which one the Bear reflects and why. This is the equity-side permanent-impairment scenario; the master synthesizer carries it into §24 / Kill Criteria. (Still no probabilities — that is the master synthesizer's job.)
