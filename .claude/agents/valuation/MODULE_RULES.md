...
| **Stale pool-verified price** (as-of date known and > **10 trading days** before the run date) | The price stays `pool-verified` (margin of safety / downside-to-bear still unlock — they are anchored to it), BUT: **valuation confidence max 70**, and `01` must document a **refresh attempt** (prefer a fresher pool/user quote; if none, proceed with the stale price explicitly flagged). If as-of is > **20 trading days** stale: **valuation confidence max 60** and the price-relative reads (margin of safety, downside-to-bear) carry a mandatory inline staleness flag stating the as-of date and drift risk. *(Quantitative teeth for the prior prose-only caveat — a 12-day-old anchor materially moved the EMAAR margin of safety. Trading days ≈ calendar days × 5/7.)* |
