...
- **balance-sheet-survival/01_capital-structure-and-leverage.md** (cross-module, if available) — the CANONICAL gross-debt / net-debt figure for this run, built directly from the filing's own debt note (source hierarchy §4: filings beat vendor exports). Prefer this over a data-vendor "Total Debt" aggregate for the EV bridge and the Net Debt & Leverage Snapshot (§4/§5 below) — a vendor aggregate frequently folds in operating-lease liabilities the company's own debt note does not classify as debt.
