...
The pipeline will discover agents at `.claude/agents/valuation/[0-9][0-9]_*.md`, group them by `layer`, dispatch each layer in parallel, strip confirmation blocks, write outputs to `<RUN_ROOT>/valuation/`, and apply fail-fast checks.
