...
Step 8A performs the deterministic half. Step 9B now performs the same final audit/reconciliation contract for every thesis-producing rerun; no entry point may publish unchecked synthesis merely because the initiating work was one orb.
...
## 9B. Final LLM audit trio + RUN_METADATA (every thesis-producing rerun)

This step is mandatory whenever Step 8 wrote `final_thesis.md`, including standalone master, module,
and single-orb reruns. Reuse `/research:full`'s shared finalization procedure below, including the bounded
evidence repair and reconciliation in 10B.3A. The `.defer_module_memos` marker controls module-memo
scheduling only; it must never exempt a published thesis from evidence verification. This deliberately
adds the audit cost to a thesis-producing rerun; it does not repeat unrelated specialist work.
...
Step 9B already ran this gate before sealing for every thesis-producing rerun; **do not rerun it against
the immutable record**. Only if no thesis was produced and the record remains unsealed, validate the
record before any commit against the versioned data-needs contract and today's self-discovered roster:
...
- Step 8A's deterministic finish-gate runs for EVERY rerun, standalone or chained — never skip it just because `.defer_module_memos` is absent. The LLM audit trio (verify-evidence + pre-mortem + expectations-gap, Step 9B) and final reconciliation also run for every thesis-producing rerun.
