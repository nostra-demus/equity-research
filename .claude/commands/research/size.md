...
`$ARGUMENTS`: empty/`all` → the **standing, corrected** ledger; a ticker → that name. Discover records by running `python3 scripts/ledger_records.py --standing-json` (Bash) — authoritative per `frameworks/DECISION_LEDGER.md` §4a: it **drops superseded runs** (a corrected-away call must never receive a paper-portfolio weight) and **applies append-only errata on read**. Iterate its `[{run_root, record, integrity}]` entries; do NOT raw-glob `analyses/*/decision_record.json`. For each, also read the **latest-version** audit reports (`expectations_gap*.json` → edge; `pre_mortem*.json` → confidence haircut & verdict). Resolve "latest" by highest `_vN` (base = first). For the **truth-integrity** gate, use the resolver's own `entry.integrity.status` (`verified` / `provisional` / `unaudited`) — the SAME banner-aware signal `/research:calibrate` and `/research:track` read — **not** the raw `verification_report*.json` verdict: a run can carry a Clean report yet still be `provisional` because the `/research:full` finish-gate stamped `final_thesis.md` PROVISIONAL (a broken scenario/edge/scorecard check), and a report-only read would let that flagged run take a paper-portfolio weight that calibrate and track both refuse it.

## 2. Eligibility (what may carry model weight)

- **Long-eligible** only if ALL hold: basket = **Selected** (Strong Buy / Buy / Starter); probability-weighted `expected_return_pct` > 0; an **evidence-backed edge** (`expectations_gap.is_exploitable` true, or `edge_score` ≥ ~50 with variant-perception ≥ Moderate); no rating cap below Buy; **truth-integrity `entry.integrity.status` ≠ `provisional`** (a flagged run — unverified or an active finish-gate PROVISIONAL banner — is not sized, mirroring calibrate/track; an `unaudited` run is NOT penalised); pre-mortem verdict ∉ {Does not survive, Thesis broken}.
- **Short Candidate** → paper short (symmetric logic), only if the thesis explicitly supports it — and subject to the same `entry.integrity.status` ≠ `provisional` gate as a long.
