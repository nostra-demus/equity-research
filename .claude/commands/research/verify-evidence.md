...
Two anchors fail most often and get a HARD assertion: **(a) strict net cash/debt** must equal `cash & equivalents − total debt` ONLY (§15) — flag as a mislabel any figure tagged *"strict"* that nets in deposits or short-term investments (that is broad), and flag as an anchor conflict the same *"strict"* label carrying two or more different values across modules (the TMCV run had four: ₹2,082 / ₹2,959 / ₹7,433 / ₹13,713, all tagged "strict"). **(b) EBITDA provenance** — if modules carry different EBITDA values, OR the same value attributed to different source *types* (a data-vendor standardized EBITDA in one module, the filing's underlying EBITDA in another), that is an anchor conflict: name both the values AND their provenance, since a 3–4% vendor-vs-filing gap silently re-rates every leverage and EV/EBITDA ratio downstream.
