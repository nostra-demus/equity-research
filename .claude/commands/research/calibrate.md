---
description: Compatibility wrapper around the deterministic decision-ledger scoreboard — global and provenance-aware provider/profile calibration, exact benchmark-adjusted hit rate, Brier + Murphy decomposition, Selected−Rejected spread, sequential e-value, and months-to-significance. Refuses every unsupported skill claim.
argument-hint: [SCOPE]
...
- `calibration_by_provider`, `calibration_by_execution_profile`, and a non-blended `provider_comparison`.
  Forecast/Brier rows follow the stamped terminal `decision_author`; directional and basket outcomes follow
  the whole-pipeline cohort (`claude`, `codex`, `mixed`, partial, or legacy/unknown). The same integrity gates,
  exact horizons, N floors, and five-distinct-ticker Brier floor apply independently to each slice. Missing
  historical provenance is never silently guessed. Mixed, configured-only, partial, and legacy rows remain
  visible but cannot enter the recorded Claude-versus-Codex comparison. No provider ranking or provider-specific
  confidence haircut is produced by this schema version.
...
- the Claude and Codex rows separately from `provider_comparison`, including each row's N/ticker floor state;
  explicitly refuse a blended provider hit rate or ranking until both recorded cohorts independently clear the
  applicable floor, and keep mixed/partial/configured/legacy cohorts named as exclusions;
