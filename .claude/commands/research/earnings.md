...
You are the orchestrator for the earnings module. The ticker is `$ARGUMENTS`. This module's specialists live in `.claude/agents/earnings/` and are organised into ordered layers via `layer:` frontmatter. You must discover them — never hardcode their names.
...
```
mkdir -p analyses/$ARGUMENTS_<DATE>/earnings
```
...
Use the Glob tool with pattern `.claude/agents/earnings/[0-9][0-9]_*.md` to list every agent file in this module.

For each matched file:

1. Parse the basename to extract:
   - `<NN>` — the two-digit prefix (e.g. `00`, `04`, `99`)
   - `<name>` — the slug between `<NN>_` and `.md` (e.g. `earnings-data-triage`, `earnings-synthesis`)
...
If the glob returns zero matches, STOP and tell the user: "No earnings agents found at `.claude/agents/earnings/[0-9][0-9]_*.md`."

## 5. Group agents by layer

Group the discovered agents by their `layer` field. Sort the layer keys ascending (0, 1, 2, …). The synthesizer (`99_earnings-synthesis.md`, `layer: 4`) is treated as the final layer regardless of how many other layers exist — this falls out of ascending sort because it has the highest layer number, but rely on the sort, not on the specific number `4`.

## 6. Resolve business-model cross-module path
...
```
ls -1d analyses/$ARGUMENTS_*/business-model/ 2>/dev/null | sort -r | head -n 1
```

Capture the result as `<BUSINESS_MODEL_PATH>`. The `sort -r | head -n 1` selects the latest folder by directory name, which sorts correctly thanks to the `YYYY-MM-DD` date format embedded in the path.

If the command returns an empty string (no prior business-model run for this ticker), set `<BUSINESS_MODEL_PATH>` to the literal string `not available`. The path must be passed verbatim to every agent's Task message in Step A below. Agents that do not need the cross-module data will simply ignore it; per `.claude/agents/earnings/MODULE_RULES.md`, agents that need it will fall back to independent analysis when it equals `not available`.

## 7. Execute layers in order
...
- User message (substitute `$ARGUMENTS`, `<DATE>`, and `<BUSINESS_MODEL_PATH>` literally):

  > Analyze ticker $ARGUMENTS. Data pool path: data/$ARGUMENTS/. Today's date: <DATE>. Business-model cross-module path: <BUSINESS_MODEL_PATH>. Follow your system prompt and produce your complete report as your final assistant message, formatted exactly per your REPORT STRUCTURE section. Return the report inline — do not attempt to write any files.
...
- Determine the output path: `analyses/$ARGUMENTS_<DATE>/earnings/<NN>_<name>.md` (using the `<NN>` and `<name>` parsed during agent discovery in step 4).
...
```
git add analyses/$ARGUMENTS_<DATE>/earnings/
...
- Full path to the synthesizer's output: `analyses/$ARGUMENTS_<DATE>/earnings/99_earnings-synthesis.md` (or note that it did not run, if aborted)
...
- Adding a new file like `.claude/agents/earnings/13_supply-chain.md` with `layer: 2` in its frontmatter must require zero changes to this orchestrator — it should automatically be picked up, run in layer 2, and written to `analyses/$ARGUMENTS_<DATE>/earnings/13_supply-chain.md`.
