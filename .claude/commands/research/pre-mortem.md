...
**Information-partition rule (no look-ahead in the haircut).** A pre-mortem's job is to stress the reasoning the engine had at `decision_date`, and its output can only ever HOLD or LOWER conviction. When this pass runs **after** the decision (a backfill, or a re-run on a still-open thesis), any confidence haircut it recommends must rest **only on information knowable on or before `decision_date`** — a base rate, a disconfirming fact that was already visible, a weak link in the original evidence. A fact dated **after** `decision_date` (the risk actually materialised, the catalyst actually missed) is an **outcome**, owned by `/research:review-decisions`, and must NOT feed a retroactive confidence haircut here — grading the decision by its result is the "resulting" error §8/§10 exist to prevent. Outside-view base rates (which are timeless) are always fair game.
