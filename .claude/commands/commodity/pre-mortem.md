...
Confirm `<RUN_ROOT>/decision_record.json` exists and `<RUN_ROOT>/commodity-thesis/99_commodity-thesis-synthesis.md` exists; if either is missing, STOP and report "No finished commodity run at `<RUN_ROOT>`." Capture `<COMMODITY>` (from `decision_record.json`'s `commodity` field) and `<DECISION_DATE>` (the decision date the run recorded — the information cutoff this whole pass is bound to).

**Bind the report to the exact decision it audited.** Because `commodity/runs/<COMMODITY>/` is a stable, single-folder location that `/commodity:rerun` overwrites in place, paths alone cannot tell a later reviewer *which* decision a report assessed. So also capture, and later persist in the report (step 5): `<DECISION_DATE>` and an immutable content identity of the audited record — `<DECISION_RECORD_SHA256>` = `sha256sum <RUN_ROOT>/decision_record.json` computed now, at read time. A subsequent rerun that rewrites `decision_record.json` produces a different hash, so `/commodity:review` and calibration can always confirm which decision (and which information cutoff) this pre-mortem graded, even when `original_action`/`original_confidence` happen to repeat.
...
5. **Fair-value / margin-of-safety attack** (`fair_value_attack`) — is the bear/base/bull band real, or does it rest on a thin or dated cost-curve orb? Does the stated margin of safety survive a plausible cost-curve shift (a new low-cost producer, an FX move in a major producing country, a demand-destruction threshold reached earlier than modeled)? If the fair-value orb was absent from this run (margin of safety "Not assessable"), say plainly that the run **lacks a valuation anchor** — but do NOT overstate this as "rests on curve/positioning alone": the terminal thesis still reads and depends on the supply-demand (balance) and macro-positioning syntheses, which stand on their own. Weigh only the *missing valuation anchor* into the haircut (a real, bounded penalty — no cost-curve floor to bound the downside), not a blanket discount of the balance, macro, and positioning evidence that is present. Manufacturing an excessive haircut here would violate the "do not invent weaknesses / do not manufacture drama" rule.
...
7. **Relative-attractiveness attack** (`relative_attack`) — does the dossier's "are we in the right commodity" comparison hold up, or does it cherry-pick an easy neighbour while ignoring a more attractive alternative in the tracked set? Use `frameworks/commodity/COMMODITY_PROFILES.md` ONLY to enumerate the tracked set — its header states it carries no live prices or estimates, so its static lens/instrument descriptions cannot establish that another commodity is *currently* more attractive. To make an actual relative claim, read the tracked peers' own as-of run artifacts within the decision-date boundary — `commodity/runs/<PEER>/decision_record.json` and their module syntheses, restricted to information knowable on or before `<DECISION_DATE>` (§3, and the information-partition rule above). If those peer artifacts do not exist or are not as-of-comparable, state `Not proven from available data.` rather than inventing a peer comparison — an uncited relative claim must not drive the verdict or the haircut.
...
Syntax is necessary but **not sufficient** — `json.tool` checks only that the file parses, so a missing field, a string where a boolean/number belongs, an unsupported verdict/action, or a miscomputed `recommended_confidence` would still "pass" and ship a malformed audit that the planned finish-gate and calibration loop cannot safely consume. So run **both** a syntax check and the cross-field semantic checks, and only continue when both pass:

```bash
python3 -m json.tool "<report_file>" >/dev/null && echo "OK valid JSON" || echo "FAIL invalid JSON"
```

Then confirm every one of these **semantic invariants** holds (pinned to this command's own contract — rule 1, step 2's action-cap ordering, and step 4's formula; fix and rewrite the report if any fails):
- **All keys present** — every field in the schema above is present (use `""`/`null`/`[]`/`{}` for genuinely-unknown values, never omit the key).
- **Types** — `original_confidence`, `confidence_haircut`, `recommended_confidence` are numbers or `null`; `survives`, `fair_value_attack.band_present`, `fair_value_attack.margin_of_safety_durable`, `carry_attack.durable` are booleans or `null`; the `[]` fields are arrays.
- **Enums** — `verdict` ∈ {`Survives`, `Survives with haircut`, `Does not survive — downgrade`, `Thesis broken`}; `original_action` and `recommended_action_cap` ∈ {`Buy`, `Hold`, `Trim`, `Avoid`, `Research More`} (`recommended_action_cap` may also be `""` for "no cap beyond the run's own action").
- **Haircut arithmetic** — `confidence_haircut ≥ 0` and `recommended_confidence == max(0, original_confidence − confidence_haircut)` (rule 1: never negative, never above `original_confidence`).
- **Verdict ↔ survival consistency** — `survives == true` iff `verdict` ∈ {`Survives`, `Survives with haircut`}; `survives == false` iff `verdict` ∈ {`Does not survive — downgrade`, `Thesis broken`}. `Survives` (no haircut) requires `confidence_haircut == 0`.
- **Cap direction** — `recommended_action_cap` is never *less* cautious than `original_action` on the ordering `Buy → Hold → Trim → Avoid` (rule 1); `Research More` is only used for the "insufficient evidence" outcome.
...
Pass the **exact `<report_file>` selected in step 5** — not a `pre_mortem*.json` wildcard. The wildcard would sweep in every prior versioned report in the folder (re-attributing an older `_v2`/`_v3` audit to this commit's message, and — if another invocation's file were present — staging work this run did not produce). Stage only what this invocation wrote:

```bash
bash scripts/commit-run.sh "Commodity pre-mortem: <COMMODITY> — <verdict> (conf <original_confidence> -> <recommended_confidence>)" -- "<report_file>"
```
