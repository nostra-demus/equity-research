...
- **Human override of the gate (checked BEFORE the terminal-stop rule).** If the module is `signal-gate`, its routing is `PARK` or `LOG`, AND `<RUN_ROOT>/intake.json` has `"override_promote": true`, treat the routing as a **continue** — a recorded human override of the promotion gate. Do NOT stop: proceed to the next module, and note `signal-gate override: <PARK|LOG>→PROMOTE (human override_promote)` in RUN_METADATA. This is what makes a human "Override & run" work even when signal-gate is REUSED from disk: its synthesis is skipped on a resume, so the override cannot be applied inside signal-gate (which only upgrades PARK anyway) — it is applied HERE at the routing gate, and it upgrades both PARK and LOG. Downstream state readers must therefore treat a signal that reached a locked thesis as having progressed past the gate, even if `signal_payload.routing` still records the original PARK/LOG.
