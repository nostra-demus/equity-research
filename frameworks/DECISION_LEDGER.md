...
6. If **any** module (step 3) OR **any** forecast type (step 4) OR **any** thesis type (step 5) is flagged: `status = "applied"`. Apply a single, fixed **8-point confidence haircut** — one bounded, auditable constant shared by all three triggers, never additive (a module flag, a forecast-type flag, and a thesis-type flag together still cost 8 points, not 24) and never stacked per extra flag within any list. Name every flagged module in `modules_flagged`, every flagged forecast type in `flagged_forecast_types`, and every flagged thesis type in `flagged_thesis_types`, each with the Brier score / hit-rate numbers that flagged it, in both the Pre-Write Gate output and the Confidence Scoring Rules "Additional downgrades" list.
7. If checked (steps 3, 4, and 5 all ran against real signal) and nothing is flagged in any of them: `status = "checked_no_action"`. No haircut — but the check still ran and is recorded, so a silent skip is distinguishable from an honest clean check.
