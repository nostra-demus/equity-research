...
| `margin_of_safety_pct` | Recommended (additive) | Discount of price to the **base-case** fair value = `(base FV − price) / base FV`; `null` when there is no pool-verified price ("Not assessable"). Direction-uniform (built from price levels, not returns) — a short candidate yields a negative MoS. The eval harness re-derives it from the `base`-labelled `scenarios[]` target (check M); only reconciled when present. Backward-compatible — older records omit it. | Part I / valuation |
...
**`margin_of_safety_pct` is additive** (introduced 2026-07-10) — the valuation module's margin of safety, `(base FV − price) / base FV`, made machine-readable so the eval harness (check M) can re-derive it from the `base`-labelled `scenarios[]` target instead of trusting the prose. It is `null` whenever there is no pool-verified price ("Not assessable"), so the gate only reconciles a PRESENT value. Records dated before 2026-07-10 omit it; downstream consumers treat absence as unquantified. `schema_version` stays "1.0" — the same additive convention as `scenarios[]` / `edge_score`.
