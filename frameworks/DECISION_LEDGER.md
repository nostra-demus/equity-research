...
**What it does not yet do.** The cockpit's live `GET /api/calls` endpoint (`ui/server/src/outputs.ts`) is a separate TypeScript implementation of the same call-row shape and does not yet read `verification_report.json` / the PROVISIONAL banner — only the Python-side `/research:track` command and `scripts/calibrate.py` are wired. Mirroring this into the TS live view is the natural next step (see CLAUDE.md §26's zero-touch spirit — this is engine code, so it goes through the normal branch → PR → CI → review path, per §28).
