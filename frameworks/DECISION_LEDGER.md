...
8. If checked (steps 3, 4, 5, and 6 all ran against real signal) and nothing is flagged in any of them: `status = "checked_no_action"`. No haircut — but the check still ran and is recorded, so a silent skip is distinguishable from an honest clean check.
