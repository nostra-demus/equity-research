...
5. If **any** module (step 3) OR **any** forecast type (step 4) is flagged: `status = "applied"`. Apply a single, fixed **8-point confidence haircut** — one bounded, auditable constant shared by both triggers, never additive (a module flag plus a forecast-type flag still costs 8 points, not 16) and never stacked per extra flag within either list. Name every flagged module in `modules_flagged` and every flagged forecast type in `flagged_forecast_types`, each with the Brier score / hit-rate numbers that flagged it, in both the Pre-Write Gate output and the Confidence Scoring Rules "Additional downgrades" list.
6. If checked (steps 3 and 4 both ran against real signal) and nothing is flagged in either: `status = "checked_no_action"`. No haircut — but the check still ran and is recorded, so a silent skip is distinguishable from an honest clean check.
