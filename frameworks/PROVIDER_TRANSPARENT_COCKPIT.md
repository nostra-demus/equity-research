...
- **A publication the supervisor refused for a reason no retry can change is never continued automatically.**
  Today that is the data catalogue's FAIL verdict on the proposed tree (it judges the whole index, so one
  uncatalogued path refuses every run until it is fixed). The refusal is signalled by a typed error (raised
  from `scripts/commit-run.sh`'s reserved exit code `7`, or by any supervisor gate that opts in) and
  recorded by the single close finalizer as the distinct durable reason `publication_refused` — never
  inferred from an error message. Every automatic lane holds it as Needs attention: the protected
  full-chain journal, the legacy `.interrupted` scan, screener signals, constellation swarms, the bounded
  doc-intake retry, and the browser's own auto-resume, which honours an explicit `autoResumeDue: false` for
  any reason. Continuing it would delete the finished thesis and pay for the terminal master again only to
  meet the same refusal. No automatic writer may downgrade the `.interrupted` marker that records it. The
  manual Resume stays available once the cause is fixed — including after the master has already written
  its thesis, where the planner calls the saved run complete: that Continue is handed to the same protected
  chain lane with the hold lifted, under the run's frozen profile. A transient publication failure (push,
  network, a lost remote race, a validator that never ran) keeps `publication_failed` and keeps
  auto-resuming. The rule is identical under every provider; `test/publication-refusal-resume.test.ts` pins it.
