...
The master orchestrator derives both run order and cross-module context from each module's `depends_on:` frontmatter (a list of module names) on its `99_<module>-synthesis.md`. Declare your new module's dependencies there — e.g. `depends_on: [business-model, earnings]`, or `depends_on: []` for a foundational module with no upstream reads. `/research:full` then topologically sorts your module into the run (a module runs after all its dependencies; ties broken alphabetically) and auto-builds its cross-module context: one `<Dep> cross-module path: <path>.` sentence per dependency that completed in the run (dependency name, first letter capitalized — e.g. `Business-model cross-module path: …`). No edit to the orchestrator is needed. Your dependent agents should parse the `<Dep> cross-module path:` label(s) for the modules they read (see `frameworks/MODULE_PIPELINE.md`).
