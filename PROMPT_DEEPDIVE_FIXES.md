...
## DD-17 · Valuation cross-file convention collapse (external PR#10 review — ceekay-munshot)

**Files:** `valuation/MODULE_RULES.md`, `01`, `04`, `07`, `99`, `README.md`; `earnings/04_guidance-consensus.md`; `PROMT_AUDIT_CHECKLIST_BANKS.md`; `READINESS_GATE_DESIGN.md`; `.claude/tools/test-tools.sh` · commit `22cc1a9` (PR#10 branch) · **two review rounds, both verified real against the files before any patch**

**Issue (one disease, not four bugs).** A reviewer re-read all 24 changed files together and found the valuation prompts internally contradictory in four places — and the root cause was singular: **a cross-file convention changed on one side of a contract and not propagated, with no cross-file consistency gate.** This is the blind spot the per-prompt sweep (DD-01…16) structurally can't see — it audits one prompt at a time, but these bugs are only visible *across* prompts. Symptoms, all confirmed on-branch: (a) **margin of safety defined three ways** — `MODULE_RULES` Calc-Std 11 said `(FV−price)/FV`, but Core-Principle 4, the scores row, and `07` said *distance to the bear case* (which also made it redundant with the Downside-risk score); (b) **point-vs-range not migrated** — DD-09 made the base case a POINT in `07`, but `04`, `99`, `MODULE_RULES`, the README, and the checklist still called it a "range"; (c) **the no-price cap contradicted itself across four encodings** (Data-Gaps row / Score-Cap row / `99` prose / `99` cap-table) over *which* score it capped — attractiveness vs confidence, one row self-contradictory; (d) **`07` weighted methods its own producers had disowned** — `02` short-history reversion ("illustrative-only") and `06` collapse ("sanity-check only") were still in `07`'s hard-coded "weights sum across 02,03,04,06."

**Fix (market-standard canonical choices, each stated ONCE then referenced — the DRY collapse).**
- **Margin of safety** = `(base FV − price)/base FV` (the cushion, Graham sense); **downside-to-bear** = `(price − bear FV)/price`, a *separate* **inverted** metric. Removes the triple definition and the Downside-risk redundancy.
- **Output shape:** bull/base/bear are POINTS; "range" reserved for the football-field dispersion / bull-to-bear spread. Swept `04`, `07`, `99`, `MODULE_RULES`, README, checklist.
- **Price-state:** ONE canonical no-price Score-Cap row (MoS / downside-to-bear / observed up-down / attractiveness = "Not assessable" **and** confidence ≤ 55); `01` emits a `pool-verified` / `indicative` / `none` tag that `07` and `99` read. Implemented as a **binary** gate (indicative = none for scoring), **not** the reviewer's 3-state machine — the treatment is identical, so three paths would add structure without behaviour.
- **Method weighting:** `07` zero-weights any method invalid for the business type **or** self-flagged illustrative-only / collapsed / sanity-check; the hard-coded "02,03,04,06" becomes "value-producing **and** valid for the type" (fixes financials/REITs too).
- **Docs/micro:** checklist synced to the point+split shape; `READINESS_GATE_DESIGN` "missing `pdftotext`" → "no usable extraction path" (post-DD-16); `earnings/04` verdict template shows the optional `provisional` qualifier — **internal-consistency only**, after verifying **no parser consumes the string** (declined the reviewer's "allowed-strings enum" as fixing a hypothetical).

**Other angles stress-tested and CLEARED (not invented).** Net-debt (`04`/`06` cite `01` verbatim), share count (same), and WACC (`05` inverts `04` verbatim, fix F11) are already DRY — three candidate "missed angles" verified as non-gaps rather than asserted. The sharpening: the valuation *numeric anchors* were consciously DRY'd; only the *definitions/shapes* drifted because they had no single home — so the prevention is to give the definitions the same "state once, reference" treatment the numbers already had.

**Prevention (right-sized).** One regression-guard added to `test-tools.sh`: asserts the old drift phrasings (MoS-as-bear-distance, base-case-as-a-range) don't return and the canonical definitions exist once in `MODULE_RULES`. Honestly labelled a prose-lint — weaker than the code tests — whose only job is to stop *this* drift recurring; the real prevention is the DRY collapse, not the grep. **No** cap-registry, 3-state machine, or consistency framework.

**Risk read.** Convention + cross-agent-consistency only (same class as DD-09/DD-12), no DAG change. Gates: smoke suite 5/5 PASS (incl. the new guard); eval PASS (3 golden runs + framework contracts J); banned-phrase clean. The lesson for the ledger: per-prompt auditing is strong for localized structural bugs (the reviewer cleared every DD-01…16 fix) and blind to module-wide *semantic conventions* — the regression-guard + DRY collapse close that specific gap.
