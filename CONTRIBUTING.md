...
So a test that finalizes a broken run or reviews a call installs the seam that exists for it (`__setFailureNoteCommitter`, `__setSupervisorCommitter`, `__setPostReviewCalibration`). Code with no seam — the Ideas publisher, the watchlist and module-checkpoint routes — is pointed at a sandbox repository under `os.tmpdir()` holding a **stub** helper, as `ideas-publisher.test.ts` does; injecting a fake `command` is not enough on its own, because the repo root is what the guard checks. The real helper refuses under `ENGINE_TEST_RUN=1` even inside a sandbox, so a test that deliberately drives it end to end removes the variable for that one child. `ENGINE_NO_PUSH=1` is not a substitute: it stops the push but still commits on your branch. One limit: a test file run directly that imports no server code and spawns a non-test entry point is covered only by the suite runner, so `npm test` and CI are the authoritative run. `test/test-publication-guard.test.ts` holds all of this in place.
