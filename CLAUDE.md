...
- **Code — everything else.** Engine source (`ui/server/src/**`, `ui/web/**`), the prompt-program (`.claude/agents/**`, `.claude/commands/**`, `frameworks/**`), the doctrine files (`CLAUDE.md` and its twin), build, CI, and scripts (`scripts/**`, `.github/**`), and root `*.md`. **Code MUST go through branch → pull request → green CI → multi-view adversarial review → merge. It may NOT be pushed directly to `main` by any contributor, human or AI.** The agent, command, and framework `.md` files ARE the program — a bad edit changes research behaviour with no compiler to catch it — so they are gated exactly like source code. **The review gate is automated, not a human sign-off:** the standing bot reviewers (CodeQL, Gemini, Codex, Copilot) plus the engine's own multi-lens adversarial pass are the bar; no human approval is required, and the engine's PR agent self-merges once that bar is green.

**Why two streams.** Research data is high in volume, low in risk, and machine-generated; gating it behind review would stall the engine and train reviewers to rubber-stamp. Code is low in volume, high in risk, and changes behaviour for every future run; it has to be adversarially reviewed and tested before it lands — by multiple automated reviewers whose findings are triaged and resolved, not by a human bottleneck. One repository, two rules.

**Autonomous merge authority.** The engine's PR agent owns a code PR end to end and needs no human to advance it. It is standing-authorized to: update an out-of-date branch (rebase or merge the base itself), resolve merge conflicts, run the multi-view adversarial review (the bot reviewers above plus its own multi-agent lenses), triage every finding — fix the real ones, record a reasoned won't-fix for the rest — and merge once CI is green and the review is clean. The `needs-human` label is advisory context, never a blocker. "Bulletproof before merge" means the multi-reviewer bar was actually cleared — not that a person clicked approve. Retaining the multiple independent views (each bot reviewer, plus the engine's own adversarial lenses) is the point: they make a change bulletproof; the human click did not.
...
2. A GitHub branch ruleset on `main` requires a pull request for every code change and runs CI as a required check. The engine's identity is the bypass actor, doing double duty: `commit-run.sh` publishes research **data** straight to `main` (§25), and the engine's PR agent **merges** a code PR once it is green and adversarially reviewed. The ruleset carries **no required-human-approval rule** — the gate is CI plus the automated multi-reviewer pass, run and cleared by the engine, not a person. The engine still never pushes *code* straight to `main`: `commit-run.sh` stages only data (point 1), so code lands only by merging a PR that carried CI + review. Every non-engine push likewise goes through a PR. The exact GitHub settings and the contributor workflow live in `CONTRIBUTING.md`.
...
**⚠️ Temporary (2026-07-23 → expected 2026-08-07): CI cannot run.** GitHub Actions is disabled org-wide by a billing lockout — the owner is disputing an incorrect invoice and has stopped paying; a downgrade to the Free plan lands 2026-08-07 and is expected to restore it. Until then **every** job on **every** branch fails in ~1–2 seconds with **no logs**. That red is an outage signature, not a code defect: never read it as a PR failure, never "fix" the workflows to chase it, and never let it stall a merge. The §28 bar still has to be cleared — clear it by running the equivalent jobs locally (`.github/workflows/ci.yml` is the authority on which), triaging the bot reviewers (they still post), and noting in the merge trail that CI was verified locally. `CONTRIBUTING.md` carries the full runbook. Delete this note once Actions is healthy again.
