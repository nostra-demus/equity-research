...
test('synthetic live-run replay stays within the cockpit paint budget without launching work', async ({ page }) => {
  ...
  expect(result.projectedRows).toBe(240)
  expect(result.renderedRows).toBe(240)
  ...
  expect(apiRequests, 'the production-path replay is entirely local and must not call the engine').toEqual([])
})
