// The detail panel's trigger reading rules (lib/watchlistView.ts).
//
// The cases that matter are the ones where the two units meet: a dated trigger must never be expressed as
// a percentage, a price trigger must never be expressed in days, and a target and its distance must always
// describe the same trigger. Run: npx tsx src/lib/watchlistView.test.ts
import assert from 'node:assert/strict'
import type { WatchRow, WatchTriggerEval } from './types'
import { absenceReason, distanceLabel, rowDistance, triggerTarget, nearestTarget, stillToMove } from './watchlistView'
