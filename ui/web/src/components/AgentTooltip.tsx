...
function hint(node: AgentNode, status: NodeStatus): { text: string; go: boolean } {
  switch (status) {
    ...
    case 'skipped':
      return { text: 'Skipped — upstream decision surface unchanged · click to view the standing output', go: true }
    case 'running':
...
function timeLine(status: NodeStatus, startedAt?: number, endedAt?: number, expectedMs?: number, now?: number): string | null {
  ...
  if (status === 'skipped') return 'not re-run — its refreshed upstream carried an identical decision surface'
