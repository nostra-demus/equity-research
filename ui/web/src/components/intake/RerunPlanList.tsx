...
export function RerunPlanList({
    ...
    return (
      <div className="iplan iplan--none">
        ...
        {batchSupported && !blocked && <>
          <button className="iplan__run iplan__run--ghost" onClick={onRun} disabled={running || launchBlocked} title={launchBlocked ? 'Engine update in progress — runs resume when it finishes' : undefined}>
            Re-run anyway…
          </button>
  ...
  if (batchSupported && confirming) {
    return (
      <div className="iplan">
        ...
        <button className="iplan__run" onClick={onRunScoped} disabled={running || scopedPending || launchBlocked} title={launchBlocked ? 'Engine update in progress — runs resume when it finishes' : undefined}>
        ...
        <button className="iplan__run iplan__run--ghost" onClick={onRun} disabled={running || scopedPending || launchBlocked} title={launchBlocked ? 'Engine update in progress — runs resume when it finishes' : undefined}>
  ...
  return (
    <div className="iplan">
      <div className="iplan__rows">
        {cmds.map((c) => (
          ...
          >
            ...
            {!batchSupported && (
              <button className="iplan__row-run" onClick={() => onRunOrb(c.module, c.agent)} disabled={running || launchBlocked} title={launchBlocked ? 'Engine update in progress — runs resume when it finishes' : undefined}>
                Review rerun
              </button>
            )}
          </div>
      ...
      {batchSupported ? <>
        <button className="iplan__run" onClick={() => void openScopedConfirmation()} disabled={running || scopedPending || launchBlocked} title={launchBlocked ? 'Engine update in progress — runs resume when it finishes' : undefined}>
