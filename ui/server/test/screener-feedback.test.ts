...
await check('feedback saves successfully — one valid ledger line with the right fields', async () => {
  const root = tmp()
...
await check('missing reason still works — writes with empty string, no throw', async () => {
  const root = tmp()
  const record = await submitFeedback({ event_id: 'EVT-000000000000', feedback_type: 'score_too_high' }, 'tester@x', root)
  assert.equal(record.feedback_reason, '')
  assert.equal(readAllFeedback(root).length, 1)
})

await check('invalid feedback type is rejected by the shared enum schema', () => {
...
await check('score_breakdown schema field accepts null, undefined, and an object (mirrors server.ts FeedbackBody)', () => {
...
await check('user_id unavailable (identify() local fallback) never blocks the write', async () => {
  const root = tmp()
  const record = await submitFeedback({ event_id: 'EVT-111111111111', feedback_type: 'other' }, 'local', root)
  assert.equal(record.user_id, 'local')
  assert.equal(readAllFeedback(root).length, 1)
})

await check('existing screener still loads with no feedback records — [] on a missing ledger file', () => {
...
await check('undo tombstones rather than rewriting — original stays, summary excludes it', async () => {
  const root = tmp()
  const record = await submitFeedback({ event_id: 'EVT-222222222222', feedback_type: 'score_too_low', feedback_reason: 'looks underrated' }, 'tester@x', root)
  const undone = await undoFeedback(record.feedback_id, 'tester@x', root)
  assert.ok(undone)
  ...
  assert.equal(await undoFeedback('FDB-20260101-deadbeef', 'x', root), null) // unknown id -> null, no write
})

await check('idempotency: submitting the same feedback_id twice yields one line', async () => {
  const root = tmp()
  const a = await submitFeedback({ event_id: 'EVT-333333333333', feedback_type: 'duplicate_stale' }, 'tester@x', root)
