...
try {
  ...
  const estimate = estimateFromComparableRuns({ kind: 'full', provider: 'claude', profileKey: 'claude:opus:default', swarm: 'research' })
  ...
  const thin = estimateFromComparableRuns({ kind: 'full', provider: 'claude', profileKey: 'claude:sonnet:default', swarm: 'research' })
  assert.equal(thin.source, 'unavailable')
  assert.equal(thin.minutesRange, undefined)

  const codex = estimateFromComparableRuns({ kind: 'full', provider: 'codex', profileKey: 'codex:sol-terra', swarm: 'research' })
