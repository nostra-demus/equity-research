...
fs.writeFileSync(path.join(state, 'nvidia-budget.json'), `{"date":"${today}","requests":1`)
...
const code = `import('./src/news/scheduler.ts').then((scheduler) => {
  ...
  const result = {
    ...
    tiers: diagnostics.tiers.filter((tier) => tier.id === 'groq' || tier.id === 'nvidia' || tier.id === 'anthropic-triage'),
...
for (const id of ['groq', 'nvidia', 'anthropic-triage']) {
...
assert.equal(result.status.overflow.find((tier: any) => tier.id === 'nvidia')?.ledgerUnavailable, true, 'the lightweight overflow surface marks damaged authority instead of displaying zero as real usage')
