...
check('every discovered pipeline is structurally valid (slug id, enums, SLA, output shape, clamped tier)', () => {
  for (const p of real.pipelines) {
    ...
    // A connector may serve its source_type's ceiling (the most-trusted tier it may claim) OR any tier
    // LESS trusted (higher-numbered) than that ceiling — e.g. an external_other web-scrape connector may
    // legitimately self-declare tier 10 ("reputable web source, unverified", CLAUDE.md §4 item 10), one
    // notch more conservative than the external_other floor (9). Only 5/7/9 are FLOORS; 10 is the one
    // higher (more conservative) tier a real connector in this repo currently uses (yunnan-curtailment-news).
