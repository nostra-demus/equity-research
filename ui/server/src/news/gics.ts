...
export function explainGicsOf(it: { headline?: string; headline_en?: string | null; companies?: CompanyLike[] }): GicsMatchDetail[] {
  ...
  for (const c of it.companies || []) {
    ...
    for (const entry of ALIAS_INDEX) {
      ...
      // a corroborated ticker (e.g. "bats") only counts — and is only traced — when the SAME guess vouches
      // for it another way; report the corroboration so the trace explains WHY the ambiguous ticker was
      // accepted (and, by its absence, why it was rejected for a colliding company like BATS Global Markets).
      if (tk && entry.corroboratedTickers.has(tk) && corroborates(entry, nm, country)) {
        details.push({ subSector: entry.subSector, sector: entry.sector, via: 'company-alias', term: `ticker:${tk} (corroborated)` })
      }
    }
  }
  return details
