...
## Batch 7 — handoff guards + LOW polish  ✅ committed to branch, eval green

- **F30 — same-run cross-module guard.** *Issue:* the 5 standalone module commands resolved cross-module upstream via `sort -r | head` (latest folder of ANY date), so a standalone re-run could feed last month's upstream into today's analysis. *Solution:* prefer `analyses/<TICKER>_<DATE>/` (this run); fall back to a prior-dated run only with an explicit "using prior-run upstream dated X" note. *Why better:* upstream is same-run by default; a stale cross-feed is now a visible, deliberate exception.
- **F32 — upstream-gap is a cap input, not just prose.** *Issue:* `depends_on` enforces order but a missing upstream was reported only narratively. *Solution:* the master synthesizer now treats "a module ran without its declared upstream" as a `missing_data` / confidence input and won't let such a module drive the headline unchecked. *Why better:* the gap is machine-actionable, not skimmed past.
- **F25 — extractor scientific-notation.** *Issue:* `repr()` emitted `1.5e+09` for large floats, which the verify-evidence/resolver grep can't match against a cited `1500000000`. *Solution:* expand sci-notation to fixed-point in `_fmt`. *Why better:* every extracted number stays greppable. *Verified: 0 sci-notation cells in the CRM corpus.*
- **F35 (rest) — image-only PDFs.** *Issue:* a scanned/image PDF with no text layer failed with a generic error that read like corruption. *Solution:* an explicit "image-only/scanned PDF — needs OCR" status row. *Why better:* actionable (re-export / OCR) instead of a silent mystery. *(.pptx remains a noted gap — needs python-pptx/soffice.)*
- **F27 — governance "page" offsets.** *Issue:* the governance agents cited 4–5-digit "page" numbers that are PDF byte/stream offsets, not findable pages. *Solution:* MODULE_RULES now requires the printed page number / named section. *Why better:* a citation a human can actually open.
- **F43 — business-quality band anchor.** *Issue:* the aggregate was "judgment-weighted, not a strict average" with no anchor — the loosest score in the set. *Solution:* it may not exceed the second-lowest row by more than ~20 points, and the rationale must name the weights. *Why better:* the score stays reconstructable from its evidence rows.
- **F14 — governance source-tier note.** *Issue:* the governance confidence-tier list reads as if it inverts §4. *Solution:* a note clarifying it's a governance-source heuristic (consistent with §4's module-tier allowance; the canonical vendor ordering lives in §4). *Why better:* documents intent rather than reordering and risking the module's logic. *(Reviewed as defensible, not a real inversion.)*
- **F45/F26 (stated-arithmetic drift) & F42 (cap-text drift):** F45/F26 are now *prevented going forward* by F09 (executed math) + F08 (eval check M recomputes the headline); only the historical artifacts remain (you chose to leave TMCV). F42 (an eval lint asserting synthesis cap numbers byte-match MODULE_RULES) was **deliberately not added** — it carries a real false-positive risk that could destabilise the suite for low value; the caps are single-sourced in MODULE_RULES and the J-contract already guards their presence.
