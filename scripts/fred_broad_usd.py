#!/usr/bin/env python3
"""Shared transform for the Federal Reserve nominal broad US-dollar index."""
...
def build(text: str, manifest: dict[str, Any], *, url: str | None = None):
    ...
    payload = {
        "series": manifest["series"], "as_of": as_of, "fred_series_id": SERIES_ID,
...
def main(manifest: dict[str, Any], *, filename_stem: str = "broad_usd") -> int:
    ...
    as_of, payload, sidecar = build(text, manifest, url=url)
    if args.verify:
        print(f"OK verify: {len(payload['observations'])} {SERIES_ID} rows through {as_of}")
        return 0
    path = publish_pair(
        data_root=args.data_root, subject=args.subject, provider_slug="fred",
        filename=f"{filename_stem}_{as_of}.json", payload=payload, sidecar=sidecar,
    )
    print(f"wrote {path}")
    return 0
